...
class CallDetectionServiceClass {
  ...
  private listeners: Map<string, { event: string; callback: Function; subscription?: any; nativeEventName?: string }> = new Map();
  ...
  private setupMockListeners(): void {
    if (!__DEV__) return; // Never run mocks in production
  ...
  private addNativeListener(eventName: string, handler: (data: any) => void): void {
    ...
    // We do NOT store native subscriptions in `this.listeners`.
    // The native event emitter fires -> we catch it -> we distribute to JS listeners.
    // If we add multiple native listeners for the same event, they stack up and cause duplicates/leaks.
    // Instead, we only need ONE native listener per event type.
    
    // Check if we already have a native subscription for this event
    const existingNativeSub = Array.from(this.listeners.values())
      .find(l => l.nativeEventName === eventName);
      
    if (!existingNativeSub) {
       const subscription = this.eventEmitter.addListener(eventName, handler);
       // Store under a special key for native subs
       this.listeners.set(`_native_${eventName}`, { 
          event: eventName, 
          nativeEventName: eventName,
          callback: () => {}, // Not used directly
          subscription 
       });
    }
  }
  ...
  public removeAllListeners(): void {
    // Keep native subscriptions, only clear JS callback listeners
    const keysToDelete: string[] = [];
    this.listeners.forEach((listener, id) => {
       if (!id.startsWith('_native_')) {
          keysToDelete.push(id);
       }
    });
    keysToDelete.forEach(id => this.listeners.delete(id));
  }

  // Only call this on app shutdown/destroy if needed
  public destroyNativeListeners(): void {
     this.listeners.forEach((listener, id) => {
      if (id.startsWith('_native_') && listener.subscription) {
