...
export const CallLogService = {
    getCallLogs: async (forceRefresh?: boolean): Promise<CallLog[]> => {
        ...
        try {
            // Check if already granted first to avoid activity attachment issues if already allowed
            const isGranted = await PermissionsAndroid.check(PermissionsAndroid.PERMISSIONS.READ_CALL_LOG);
            if (isGranted) return await CallLogService._loadLogs();
            ...
            if (result === PermissionsAndroid.RESULTS.GRANTED) {
                return await CallLogService._loadLogs();
            } else {
                return [];
            }
        } catch (error) {
    ...
    _loadLogs: async (): Promise<CallLog[]> => {
